...
class Database {
  ...
  async createAccount(name, input = {}) {
    const payload = {
      name,
      enabled: input.enabled !== false,
      notes: input.notes || '',
      twitterUsername: String(input.twitterUsername || '').trim(),
    };
    if (Array.isArray(input.cookies) && input.cookies.length) {
      const encrypted = encryptJson(input.cookies);
      payload.cookiesEncrypted =
        typeof encrypted === 'string' ? encrypted : JSON.stringify(encrypted);
    }
    if (input.password) {
      const encrypted = encryptJson({ p: input.password });
      payload.passwordEncrypted =
        typeof encrypted === 'string' ? encrypted : JSON.stringify(encrypted);
    }
    return Account.findOneAndUpdate({ name }, payload, {
      upsert: true,
      new: true,
  ...
  async renameAccount(oldName, requestedName) {
    ...
    if (newName.length > 100 || /[\x00-\x1f]/.test(newName)) {
  ...
  async rotateAccountProxy(requestedAccountName) {
    const account = await Account.findOne({
      $or: [{ name: requestedAccountName }, { aliases: requestedAccountName }],
    }).lean();
  ...
  async updateProxyStatus(id, status, extra = {}) {
    ...
    const patch = { ...update };
    const inc = {};
    if (status === 'dead') inc.failCount = 1;
    else if (status === 'active') { inc.successCount = 1; patch.lastError = ''; }
    const doc = await Proxy.findByIdAndUpdate(id, { $set: patch, $inc: inc }, { new: true }).lean();
    return doc;
  }
