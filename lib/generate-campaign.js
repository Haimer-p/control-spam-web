...
const { DEFAULT_COMBO_RATIOS } = require('./combo-ratios');
const { getGeminiKeyPool } = require('./gemini-key-pool');

const MIN_KEYWORDS_PER_ACCOUNT = 20;
...
const DEFAULT_INTERACTIONS = {
  ...
  replyMaxLength: 240,
  comboRatios: DEFAULT_COMBO_RATIOS,
};
...
async function callGemini(prompt) {
  ...
  return pool.executeWithFallback(async (apiKey) => {
...
async function generateKeywords(meta, options = {}) {
  ...
  const minPerAccount = options.minPerAccount || MIN_KEYWORDS_PER_ACCOUNT;
  // Sinh đủ keyword để mỗi account có tối thiểu minPerAccount (có dư để đa dạng),
  // giới hạn trong [60, 200] cho AI trả ổn định.
  const count =
    options.count ||
    Math.min(200, Math.max(60, minPerAccount * Math.max(1, accountNames.length)));
  ...
  const perAccount = distributeKeywords(allKeywords, accountNames, minPerAccount);
